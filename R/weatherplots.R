...
sebms_precip_data <- function(my_place = NA, year = lubridate::year(lubridate::today())-1) {
  ...
  all_precip <- stations %>% 
    ...
    transmute(name, id = as.numeric(id), latitud = latitude, longitud = longitude, month = month(ymd_hms(FrDate), label = T, abbr = T, locale = "sv_SE"), nb = as.numeric(nb), monthnr = month(ymd_hms(FrDate)), period = "2") %>% 
