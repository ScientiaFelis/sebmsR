...
`sebms_weather_png(2022)` creates png files with the precipitation,
temperaure figures. `sebms_sunhour_plot(2022)` creates the sun hour
figure `sebms_sundiff_plot(2022)` shows differences in sun hours between
...
To **compare a regional index** for a species with the national Sweden
index run: `get_trimComparedPlots()` creats a figure that compares the
swedish index of a species with the index for a set *Län*, *Landskap*,
or *Kommun*.
...
`sebms_sites_map()` creates map for all visited sites on Swedish grid
separating transects and point data. `sebms_distribution_map()` creates
a distribution map for species on the Swedish grid.
